...
sh ./scripts/create_and_import_account.sh
...
# Fund proof-server address if it's different from the active address
PROOF_SERVER_ENV="./crates/proof-server/.env"
if [[ -f "$PROOF_SERVER_ENV" ]]; then
  PROOF_SERVER_ADDRESS=$(grep "^SUI_SENDER_ADDRESS=" "$PROOF_SERVER_ENV" | cut -d'=' -f2 | tr -d '[:space:]')
  if [[ -n "$PROOF_SERVER_ADDRESS" ]] && [[ "$PROOF_SERVER_ADDRESS" != "$ADDRESS" ]]; then
    echo "[fund] Funding proof-server address: $PROOF_SERVER_ADDRESS"
    for attempt in $(seq 1 ${MAX_FAUCET_ATTEMPTS}); do
      echo "[fund] Proof-server faucet attempt ${attempt}/${MAX_FAUCET_ATTEMPTS}..."
      if sui client faucet --address "$PROOF_SERVER_ADDRESS"; then
        echo "[fund] Proof-server faucet request succeeded."
        break
      fi
      if [[ $attempt -eq ${MAX_FAUCET_ATTEMPTS} ]]; then
        echo "[fund] Proof-server faucet failed after ${MAX_FAUCET_ATTEMPTS} attempts." >&2
        exit 1
      fi
      SLEEP=$(( attempt * 2 ))
      if [[ $SLEEP -gt 10 ]]; then SLEEP=10; fi
      echo "[fund] Proof-server faucet attempt failed; sleeping ${SLEEP}s before retry..."
      sleep $SLEEP
    done
    
    # Wait for proof-server balance
    PS_BALANCE=0
    for i in $(seq 1 ${MAX_BALANCE_WAIT}); do
      RAW=$(sui client balance "$PROOF_SERVER_ADDRESS" 2>/dev/null || true)
      PS_BALANCE=$(echo "$RAW" | grep 'Sui' | grep -oE '[0-9]+' | head -n1 || true)
      PS_BALANCE=${PS_BALANCE:-0}
      echo "[fund] Proof-server poll ${i}/${MAX_BALANCE_WAIT} - balance=${PS_BALANCE}"
      if [[ ${PS_BALANCE} -gt 0 ]]; then
        echo "Proof-server account funded with balance: ${PS_BALANCE}"
        break
      fi
      sleep 1
    done
    
    if [[ ${PS_BALANCE} -eq 0 ]]; then
      echo "[fund] Failed to observe proof-server funded balance after ${MAX_BALANCE_WAIT}s." >&2
      exit 1
    fi
  fi
fi
